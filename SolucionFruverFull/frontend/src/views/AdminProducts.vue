      ...
      <form @submit.prevent="crear">
        <div class="grid">
          ...
          <div class="row">
            ...
            <input v-model="form.categoria" class="input" placeholder="Frutas, Abarrotes…" />
          </div>
          <div class="row">
            <label>Unidad</label>
            <input v-model="form.unidad" class="input" placeholder="kg, unidad, caja…" />
          </div>
          <div class="row">
            <label>Precio</label>
          ...
          <div class="row">
            <label>Stock</label>
            <input v-model.number="form.stock" type="number" step="1" class="input" placeholder="0" />
          </div>
        </div>
        ...
        <tbody>
          <tr v-for="p in items" :key="p._id">
            <template v-if="editId === p._id">
              ...
              <td><input v-model="editForm.unidad" class="input" /></td>
              <td><input v-model.number="editForm.precio" type="number" step="0.01" class="input" style="text-align:right;" /></td>
              <td><input v-model.number="editForm.stock" type="number" step="1" class="input" style="text-align:right;" /></td>
              <td class="actions">
            ...
            <template v-else>
              ...
              <td>{{ p.categoria || '' }}</td>
              <td>{{ p.unidad || '' }}</td>
              <td style="text-align:right;">{{ money(p.precio) }}</td>
...
// estados
const form = ref({ nombre:'', codigo:'', categoria:'', unidad:'', precio:0, stock:0 })
const items = ref([])
const error = ref('')
...
async function crear () {
  ...
  try {
    ...
    total.value++
    form.value = { nombre:'', codigo:'', categoria:'', unidad:'', precio:0, stock:0 }
  } catch (e) {
...
function cancelarEdicion(){ editId.value = null; editForm.value = {} }
